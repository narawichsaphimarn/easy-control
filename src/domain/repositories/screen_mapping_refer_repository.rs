...
impl ScreenMappingReferRepository {
    pub fn find_by_key_and_group(key: String, group: String) -> Result<Vec<ScreenMappingRefer>, Error> {
        let query = "SELECT * FROM screen_mapping_refer WHERE parameter_key = ? AND parameter_group = ?;";
